...
from mmcls.core import average_performance, mAUC, mAP
...
class MultiLabelDataset(BaseDataset):
    ...
    def evaluate(self,
        ...
        if isinstance(metric, str):
            ...
            metrics = metric
        allowed_metrics = ['mAP', 'CP', 'CR', 'CF1', 'OP', 'OR', 'OF1', 'mAUC']
        eval_results = {}
        ...
        if 'mAP' in metrics:
            mAP_value, AP_array = mAP(results, gt_labels)
            eval_results['mAP'] = mAP_value
            for i in range(len(AP_array)):
                eval_results['AP_{}'.format(i)] = AP_array[i]
        if 'mAUC' in metrics:
            mean_AUC, auc_array = mAUC(results, gt_labels, **metric_options)
            eval_results['mAUC'] = mean_AUC
            for i in range(len(auc_array)):
                eval_results['AUC_{}'.format(i)] = auc_array[i]

        if len(set(metrics) - {'mAP'} - {'mAUC'}) != 0:
