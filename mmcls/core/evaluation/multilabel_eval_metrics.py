...
def average_performance(pred, target, thr=None, k=None):
    ...
    # 测试debug使用
    # i = 0
    # print(precision_class[i], recall_class[i], 2 * precision_class[i] * recall_class[i]/ (precision_class[i] + recall_class[i]))
...
def mAUC(pred, target, thr=None, k=None):
    """Calculate AUC.
    ...
    auc_array = roc_auc_score(target, pred, average=None)
    auc_array = auc_array * 100.0
    mean_AUC = auc_array.mean()
    return mean_AUC, auc_array
